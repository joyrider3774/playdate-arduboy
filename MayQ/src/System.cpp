 ...
 * Arduboy を取得する
 */
Arduboy2Base &SystemGetArduboy(void)
{
  return arduboy;
}
 ...
 * パターンを反転して描画する
 */
void SystemReversePattern(const uint8_t *pattern, int8_t x, int8_t y, uint8_t width, int8_t flip)
{
  // 表示領域の判定
  if (x + width > 0x00 && y > -0x08 && y < HEIGHT) {
