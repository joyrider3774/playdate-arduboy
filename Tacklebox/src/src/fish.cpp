...
void Fish::loadFish(FishType fishType, Fish& fish) {
    ...
    const uintptr_t* offset16 = fish_templates_16t + static_cast<int8_t>(fishType) * NUM_16T_PROPS;

    fish.minX = (int16_t)pgm_read_word(offset16);
    fish.maxX = (int16_t)pgm_read_word(offset16 + 1);

    fish.nameString = reinterpret_cast<const uint8_t*>(pgm_read_ptr(offset16 + 2));
    fish.bmp = reinterpret_cast<const uint8_t*>(pgm_read_ptr(offset16 + 3));

    fish.minLength = (int16_t)pgm_read_word(offset16 + 4);
    fish.maxLength = (int16_t)pgm_read_word(offset16 + 5);
}
