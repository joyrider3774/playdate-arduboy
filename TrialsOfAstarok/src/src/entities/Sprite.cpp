    ...
    switch (this->getType()) {
       ...
       case ObjectTypes::Player:
            ...
            if (this->isFalling()) {
                ...
                Sprites::drawPlusMask(x - this->game->camera.x - 2, y - 1 - this->game->camera.y, pgm_read_ptr(&Images::Player_Idle[this->getFrame()]), this->facing == Direction::Right);
            }
            else {
                Sprites::drawPlusMask(x - this->game->camera.x - 2, y - 1 - this->game->camera.y, pgm_read_ptr(&Images::Player_Images[this->getFrame()]), this->facing == Direction::Right);
            }
            break;
