...
if (plusMask) {
    ...
    const uint8_t *mask_ofs = hasMask ? mask + (start_h * w) + xOffset : NULL;

    if (mirror & MIRROR_HORIZONTAL)  {
        bofs += rendered_width - 1;
        if (mask_ofs) mask_ofs += rendered_width - 1;
        if (x < 0){
            bofs -= w - rendered_width;
            if (mask_ofs) mask_ofs -= w - rendered_width;
        } else{
            bofs += w - rendered_width;
            if (mask_ofs) mask_ofs += w - rendered_width;
        }
    ...
    if (mirror & MIRROR_VERTICAL) {
        bofs += (loop_h - 1) * w;
        if (mask_ofs) mask_ofs += (loop_h - 1) * w;
        if (y < 0) {
            bofs -=  (start_h * w);
            if (mask_ofs) mask_ofs -=  (start_h * w);
        }
    }
