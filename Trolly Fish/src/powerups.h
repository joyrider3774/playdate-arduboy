...
void updatePowerUp()
{
  ...
  if (powerUp.active)
  {
    ...
    Rect playerRect = {20, trollyFish.y, trollyFish.width, trollyFish.height};

    Rect powerupRect = {powerUp.x, powerUp.y, powerUp.width, powerUp.height};
...
void checkIfScored() {
  ...
  for (byte i = 0; i < TOTAL_STARS; ++i)
  {
    Rect starFishPoint = {starFish[i].x, starFish[i].y, starFish[i].width, starFish[i].height};
