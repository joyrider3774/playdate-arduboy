...
void Arduboy2Base::flashlight()
{
    ...
    // TODO: need some sort of "is running" flag
}
...
{
    randomSeed(pd->system->getCurrentTimeMilliseconds());
}
...
{
    int end = y+h;
    for (int a = fmax(0,y); a < fmin(end,HEIGHT); a++)
    {
        drawPixel(x,a, color);
    }
}
...
// expense of slower off screen character handling.
     ...
     )
  {
    ...
    for (uint8_t i = 0; i < fullCharacterWidth; i++)
    {
        ...
        {
            column = (i < characterWidth) ? *(bitmap++) : 0;
        }
        else
        {
            column = 0;
