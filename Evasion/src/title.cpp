...
MODE_T updateTitle(void)
{
    ...
    randomSeed(rand() ^ millis()); // Shuffle random
...
static void drawSettings(void)
{
    ...
    if (isSettingChenged) {
        for (int i = 0; i < 5; i++) {
            ...
            switch (i) {
            case 0:                
                arduboy.setCursor(dx, dy);
                arduboy.printNumber(record.speed, 10);
                break;
            case 1:
                p = accelText + record.acceleration * 5;
                arduboy.printEx(dx, dy, (const char *)p);
                break;
            case 2:
                arduboy.setCursor(dx, dy);
                arduboy.printNumber(record.density, 10);
                break;
            case 3:
                arduboy.setCursor(dx, dy);
                arduboy.printNumber(record.thickness, 10);
                break;
            case 4:
...
static void mydrawText(const char *p, int16_t y)
{
    while (pgm_read_byte(p) != '\e') {
        uint8_t len = strnlen_P(p, 21);
        arduboy.printEx(64 - len * 3, y, (const char *) p);
        p += len + 1;
        y += (len == 0) ? 2 : 6;
    }
}
...
static void drawCredit(void)
{
    if (isInvalid) mydrawText(creditText, 11);
}
