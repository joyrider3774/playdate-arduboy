...
uint8_t enemyWalkingFrame;


class Enemy
{
  public:
    static const uint8_t startX = 64;
    static const uint8_t startY = 0;

    uint8_t enemyType = SMALL_ENEMY; //set enemyType to small at start
    uint8_t frameDelay = 3;
    uint8_t x, y;
    uint8_t width = 6;  // small = 6 / middle = 8 and big = 12
    uint8_t height = 6; // small = 6 / middle = 9 and big = 12
    boolean xSpeed;
    ...
    void takeDamage(uint8_t _damage) {
      HP -= _damage;
      if (HP < 1)
      {
        scorePlayer ++;
...
void checkEnemyCollisions()
{
  ...
  for (uint8_t i = 0; i < 8; i++)
  {
    ...
    if (enemy[i].isActive)
    {
      ...
      Rect bulletRect;
      for (uint8_t ii = 0; ii < weapon.maxBullets; ii++)
      {
