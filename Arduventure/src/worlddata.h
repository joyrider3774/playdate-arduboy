...
byte getRegion(byte chunk_pos_x, byte chunk_pos_y)
{
  for (byte i = 0; i < TOTAL_REGIONS; i++)
  {
    ...
    Rect reg = {pgm_read_byte(regions + i * 4), pgm_read_byte(regions + 1 + i * 4),
                pgm_read_byte(regions + 2 + i * 4), pgm_read_byte(regions + 3 + i * 4)
               };
...
void drawTiles()
{
  ...
  for (byte x = 0; x <= 128; x += 16)
  {
    for (byte y = 0; y <= 64; y += 16)
    {
...
void checkDoors()
{
  if (!(playerDirection & 0B00000001)) //WALKING NORTH OR SOUTH
  {
    ...
    for (byte i = 0; i < TOTAL_DOORS; i++)
    {
      Rect door =
      {
        (16 * pgm_read_byte(&doors[i * 2])) - 3,
        16 * pgm_read_byte(&doors[(i * 2) + 1 ]),
        10,
        10
      };
