...
uint8_t coolDown[] = { WEAPON_COOLDOWN_TRIDENT, WEAPON_COOLDOWN_BUBBLES, WEAPON_COOLDOWN_SEASHELL, WEAPON_COOLDOWN_MAGIC};
uint8_t coolDownMax[] = { WEAPON_COOLDOWN_TRIDENT, WEAPON_COOLDOWN_BUBBLES, WEAPON_COOLDOWN_SEASHELL, WEAPON_COOLDOWN_MAGIC};
uint8_t bulletSpeed[] = {BULLET_SPEED_TRIDENT, BULLET_SPEED_BUBBLES, BULLET_SPEED_SEASHELL, BULLET_SPEED_MAGIC};
uint8_t bulletDamage[] = {DAMAGE_TRIDENT, DAMAGE_BUBBLES, DAMAGE_SEASHELL, DAMAGE_MAGIC};
uint8_t bulletCollisionOffset[] = {TRIDENT_COLLISION_OFFSET, BUBBLES_COLLISION_OFFSET, SEASHELL_COLLISION_OFFSET, MAGIC_COLLISION_OFFSET};
...
struct Players
{
  public:
    ...
    uint8_t HP;
    uint8_t frame;
    uint8_t chargeBarFrame;
    uint8_t currentBullet;
    uint8_t currentSeaShell;
};
...
void checkMermaid()
{
  ...
  {
    for (uint8_t i = 0; i < MAX_ONSCREEN_SPARKLES; i++)
    {
...
void drawMermaid()
{
  ...
  if (mermaid.hasShield)
  {
    for (uint8_t i = 0; i < MAX_ONSCREEN_SHIELDS; i++)
    {
...
void setWeapons()
{
  ...
  for (uint8_t i = 0; i < MAX_ONSCREEN_BULLETS; i++)
  {
...
void drawLifeHUD()
{
  for (uint8_t i = 0; i < AMOUNT_OF_HEARTS; i++) sprites.drawSelfMasked(8 * i, 0, hearts, 0);
  for (uint8_t i = 0; i < mermaid.HP - 1; i++)sprites.drawSelfMasked(8 * i, 0, hearts, 1);
}
