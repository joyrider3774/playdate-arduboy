...
uint8_t endBossMaxHP[] = {MAX_HP_SHARK, MAX_HP_SEAHORSE, MAX_HP_PIRATESHIP};
uint8_t endBossStartY[] = {28, 16, 10};
uint8_t enemiesMaxHP[] = {MAX_HP_FISHY, MAX_HP_FISH, MAX_HP_JELLYFISH, MAX_HP_OCTOPUS, MAX_HP_SKULL, MAX_HP_SEAHORSETINY};
uint8_t enemiesPoints[] = {POINTS_FISHY, POINTS_FISH, POINTS_JELLYFISH, POINTS_OCTOPUS, POINTS_SKULL, POINTS_SEAHORSETINY};
uint8_t enemyCollisionWidth[] = {FISHY_COLLISION_WIDTH, FISH_COLLISION_WIDTH, JELLYFISH_COLLISION_WIDTH, OCTOPUS_COLLISION_WIDTH, SKULL_COLLISION_WIDTH, SEAHORSETINY_COLLISION_WIDTH};
uint8_t enemyCollisionHeight[] = {FISHY_COLLISION_HEIGHT, FISH_COLLISION_HEIGHT, JELLYFISH_COLLISION_HEIGHT, OCTOPUS_COLLISION_HEIGHT, SKULL_COLLISION_HEIGHT, SEAHORSETINY_COLLISION_HEIGHT};

uint8_t jellyFrame;
uint8_t faseTimer;
uint8_t mermaidsPosition;
uint8_t currentEnemyBullet;
...
struct Enemies
{
  public:
    ...
    int y;
    int HP;
    uint8_t characteristics;//B00000000;   //this uint8_t holds all the enemies characteristics
    //                      ||||||||
    //                      |||||||└->  0 \
...
void checkEnemyBullet()
{
  ...
  {
    for (uint8_t i = 0; i < MAX_ONSCREEN_ENEMY_BULLETS; i++)
    {
...
void setEnemies()
{
  ...
  for (uint8_t i = 0; i < MAX_ONSCREEN_ENEMIES; i++)
  {
...
void enemySetInLine(uint8_t enemyType, uint8_t firstEnemy, uint8_t lastEnemy, int x, int y, int spacingX, int spacingY)
{
  for (uint8_t i = firstEnemy; i < lastEnemy; i++)
  {
    enemy[i].characteristics = i;
    bitSet(enemy[i].characteristics, 4);
...
void enemySwimRightLeft(uint8_t firstEnemy, uint8_t lastEnemy, uint8_t speedEnemy)
{
  for (uint8_t i = firstEnemy; i < lastEnemy; i++)
  {
...
void enemySwimToMiddle(uint8_t firstEnemy, uint8_t lastEnemy, uint8_t speedEnemy)
{
  for (uint8_t i = firstEnemy; i < lastEnemy; i++)
  {
    if (!bitRead(enemy[i].characteristics, 5))
...
void enemySwimSine(uint8_t firstEnemy, uint8_t lastEnemy, uint8_t speedEnemy)
{
  for (uint8_t i = firstEnemy; i < lastEnemy; i++)
  {
    if (!bitRead(enemy[i].characteristics, 5))
...
void enemySwimDownUp(uint8_t firstEnemy, uint8_t lastEnemy, uint8_t speedEnemy)
{
  for (uint8_t i = firstEnemy; i < lastEnemy; i++)
  {
    if (!bitRead(enemy[i].characteristics, 5))
...
void enemyShoot(uint8_t firstEnemy, uint8_t lastEnemy)
{
...
void checkEndBoss()
{
  if (bitRead(endBoss.characteristics, 4))
  {
    ...
    for (uint8_t i = 0; i < MAX_BOSS_BULLETS; i++)
    {
      if (!bitRead(enemy[i].characteristics, 5))
      {
        enemy[i].x -= 1;
...
void drawEnemyHud(uint8_t currentLife, uint8_t maxLife)
{
  for (uint8_t i = 0; i < (currentLife / 3)  + 1; i++)
  {
