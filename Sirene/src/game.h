...
void stateGameNextStage()
{
  ...
  ((FunctionPointer) pgm_read_ptr (&nextstageFases[gameOverAndStageFase]))();
};
...
void stateGamePlaying()
{
  ...
  if (arduboy.everyXFrames(2)) ((FunctionPointer) pgm_read_ptr (&stages[stage - 1][currentWave]))();
...
void stateGameOver()
{
  ((FunctionPointer) pgm_read_ptr (&gameOverFases[gameOverAndStageFase]))();
...
void stateGameEnded()
{
  ((FunctionPointer) pgm_read_ptr (&gameEndFases[gameOverAndStageFase]))();
 if (objectVisible) {
    checkMermaid();
    drawMermaid();
    sprites.drawSelfMasked(35, 28, textHighscore, 0);
