...
void wave250()
{
  ...
  ((FunctionPointer) pgm_read_ptr (&sharkAttackFases[endBoss.attackFase]))();
...
void wave251()
{
  //seahorse attack
  if (checkStartWave())setEndBoss();
  ((FunctionPointer) pgm_read_ptr (&seahorseAttackFases[endBoss.attackFase]))();
  if (!bitRead(endBoss.characteristics, 7)) currentWave++;
}
...
void wave252()
{
  //pirateShip attack
  if (checkStartWave())setEndBoss();
  ((FunctionPointer) pgm_read_ptr (&pirateShipAttackFases[endBoss.attackFase]))();
  if (!bitRead(endBoss.characteristics, 7)) currentWave++;
}
...
{
  Rect mermaidRect = {mermaid.x + MERMAID_COLLISION_OFFSET, mermaid.y + MERMAID_COLLISION_OFFSET, MERMAID_COLLISION_SIZE, MERMAID_COLLISION_SIZE};
  Rect endBossRect = {endBoss.x, endBoss.y + 2, 32, 12};

  ////// Check collision bullets with enemies and bosses /////
  ////////////////////////////////////////////////////////////
  for (uint8_t k = 0; k < MAX_ONSCREEN_BULLETS; k++)
  {
    if (bullet[k].isVisible)
    {
      Rect bulletsRect {bullet[k].x + bulletCollisionOffset[bullet[k].type], bullet[k].y, 8, 8};
      for (uint8_t i = 0; i < MAX_ONSCREEN_ENEMIES; i++)
      {
        Rect enemyRect = {enemy[i].x, enemy[i].y, enemyCollisionWidth[enemy[i].type], enemyCollisionHeight[enemy[i].type]};
      ...
      if (bitRead(endBoss.characteristics, 4) && !bitRead(endBoss.characteristics, 5) && !bitRead(endBoss.characteristics, 6))
      {
        ...
        {
          case ENDBOSS_SHARK:
            endBossRect = {endBoss.x, endBoss.y + 2, 32, 12};
            break;
          case ENDBOSS_SEAHORSE:
            endBossRect = {endBoss.x + 2, endBoss.y + 2, 6, 6};
            break;
          case ENDBOSS_PIRATESHIP:
            endBossRect = {endBoss.x + 38, endBoss.y + 15, 5, 5};
            break;
        }
  ...
  {
    Rect enemyBulletRect = {enemyBullet[i].x + 1, enemyBullet[i].y + 1, 6, 6};
  ...
  if (bitRead(endBoss.characteristics, 4) && !bitRead(endBoss.characteristics, 5))
  {
    switch (endBoss.type)
    {
      case ENDBOSS_SHARK:
        ...
        break;
      case ENDBOSS_SEAHORSE:
        endBossRect = {endBoss.x + 2, endBoss.y + 2, 12, 28};
        break;
      case ENDBOSS_PIRATESHIP:
        endBossRect = {endBoss.x + 19, endBoss.y + 10, 46, 40};
        break;
    }
  ...
  if (powerUP.isActive)
  {
    Rect powerUPRect = {powerUP.x, powerUP.y, 8, 8};
...
void drawScore(uint8_t fontType)
{
  char buf[10];
